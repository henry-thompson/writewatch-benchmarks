...
int main(int argc, char *argv[]) {
	...
	double 	*array;
       ...
       if (argc >= 2) {
        kStretchTreeDepth = strtol(argv[1], NULL, 10);
       }

       if (argc >= 3) {
        kLongLivedTreeDepth = strtol(argv[2], NULL, 10);
       }

       if (argc >= 4) {
        kArraySize = strtol(argv[3], NULL, 10);
       }

       if (argc >= 5) {
        kMinTreeDepth = strtol(argv[4], NULL, 10);
       }
...
#	endif
        ...
        PrintDiagnostics();
#	ifdef GC
	  printf("%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", kStretchTreeDepth, kLongLivedTreeDepth, kArraySize, kMinTreeDepth, kMaxTreeDepth, GC_gc_no, GC_get_heap_size(), tElapsed);
#       else
